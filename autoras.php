...
<body>
  ...
  <ol>
    ...
    <li>Se usa <b>el número de páginas impresas para estimar la importancia que da la editorial a la obra y al autora/autor</b>. No es un sistema perfecto, pero es mucho más realista que simplemente contabilizar el número de obras publicadas. Cuando unas obras tiene varias personas autoras se ha dividido el número de páginas entre el números de autores a la hora de contabilizar cuántas páginas ha escrito cada autora o autor.</li>
    ...
    <table>
      ...
      <?php foreach ($res as $key => $proyecto) { ?>
        <?php if ($key > 0) { 
          ...
          if($proyecto[3]['formattedValue'] > 0 && $proyecto[4]['formattedValue']) $stats['titulos_mixtos'] ++;
          else if($proyecto[3]['formattedValue'] > 0) $stats['titulos_solo_autoras'] ++;
          else if($proyecto[4]['formattedValue'] > 0) $stats['titulos_solo_autores'] ++; 
