...
function getStars ($editorial) {
  if($editorial['proyectos'] < 5) return "no-stars"; //Para empezar hay que tener al menos 5 mecenazgos para poder evaluado.

  $stars = 0;

  if($editorial['entregados'] > $editorial['sin_entregar']) $stars++; //La primera estrella se consigue si tienes más mecenazgos entregados que sin entregar. Da igual que estén o no retrasados.

  if($editorial['entregados'] > 0 && $editorial['entregados_a_tiempo'] >= ($editorial['entregados'] / 4)) {
    $stars++; //La segunda estrella se da, si al menos un cuarto los entregados se han entregado a tiempo.

    //echo " 2-".($editorial['entregados'] / 4)." ";
  }
  $retrasado_medio = floor(($editorial['dias_retraso'] / $editorial['proyectos'])); //Se da una estrella si la media de retraso es menor de 3 meses (90 días).
  if($retrasado_medio <= 90) $stars++;

  if($editorial['max_retraso'] <= 365) $stars++; //Se otorga una estrella si no hay ningún mecenazgo con un retraso superior a un año (365 días).

  if(($editorial['entregados'] > $editorial['sin_entregar']) && ($editorial['entregados_a_tiempo'] >= ($editorial['entregados'] / 2))) $stars++; //La última estrella se obtiene, si tienes más mecenazgos entregados que sin entregar y al menos la mitad de tus mecenazgo entregados se han entregado a tiempo.
